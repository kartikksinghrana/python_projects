...
class InstagramBot:
	...
	def Login(self):
		self.driver = webdriver.Chrome("I:\Python_projects\insta_Bot2\chromedriver.exe")
	...
	def TagOptions(self):
		...
		if "y" in input("Follow the poster? (y/n):\t").lower():
			followtag = True
		# Go to tag
		self.driver.get("{}/explore/tag/{}".format(self.base_url, tag))
		self.Wait(1,2)
		...
		for i in range(amount):
			self.Wait(20,35)
			name = self.driver.find_element_by_xpath('/html/body/div[5]/div[2]/div/article/header/div[2]/div[1]/div[1]/span').text
			# Liking the post
			if liketag:
				self.Wait(1,2)
...
TestRuns = InstagramBot("your username","your_password")